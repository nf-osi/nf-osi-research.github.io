...
date: "2021-12-01"
description: |
  Status: **Standard**, Updated On: *`r Sys.Date()`*
draft: yes
output:
...
table1 %>%
  ...
  tab_style(style = list(cell_text(color = 'gray', style = "italic")), 
            locations = cells_body(columns = Requirement, 
                                   rows = !Requirement %in% c("required","sometimes required"))) %>%
  tab_options(
...
```{r fig-respondent-type, fig.cap="Demographic representation of RFC community respondents", fig.fullwidth = T, echo=F}
...
# Edges
  nodes <- types.dt %>%
    ...
    mutate(size = round(size, 0),
           label = stringr::str_wrap(demographic, 20) %>% 
             paste0(": ", size, "%"))
  ...
  ggraph(graph, 'circlepack', weight = size) +
    ...
    ggraph::geom_node_label(aes(label = label), size = 3) +
